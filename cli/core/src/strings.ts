...
import { REGEX_VAR, REGEX_VAR_ALT } from "./constants.js"

export const substitute = (template, map) => {
  const transform = (k) => k || '';
  return template.replace(REGEX_VAR, (match, key, format) => transform(map[key]).toString())
}

export const substituteAlt = (template, map) => {
  const transform = (k) => k || '';
