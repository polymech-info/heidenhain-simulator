...
import type {
	...
	IConflictSettings,
	TCopyResult,
	INodeReport } from './interfaces.js';
...
async function copyItemSync(from: string, inspectData: INode, to: string, options: ICopyOptions) {
	const mode: string = fileMode(inspectData.mode as number);
...
async function visitor(from: string, to: string, vars: IVisitorArgs, item: IProcessingNode): Promise<void> {
	...
	const checked = (subResolveSettings: IConflictSettings) => {
		...
		if (subResolveSettings) {
			...
			overwriteMode = onConflict(item.path, destPath, options, subResolveSettings) as EResolveMode
...
interface IVisitorArgs {
	...
	onCopyErrorResolveSettings: IConflictSettings | null;
}
...
export function async(from: string, to: string, options?: ICopyOptions): Promise<TCopyResult> {
	...
	return new Promise<TCopyResult>((resolve, reject) => {
		checkAsync(from, to, options).then((resolver: IConflictSettings) => {
			...
			overwriteMode = onConflict(from, to, options, resolver) as EResolveMode;
			...
			if (options && options.flags as ECopyFlags & ECopyFlags.EMPTY) {
				const dstStat = fs.statSync(to);
				if (dstStat.isDirectory()) {
					rmSync(to);
				}
			...
			const process = function () {
				...
				if (nodes.length) {
					...
					if (item) {
						visitor(item.path, item.dst as string, visitorArgs, item).then(process);
					}
				}
			};
			...
			iteratorAsync(from, {
				...
				let node: IProcessingNode;
				while (node = it.next() as any) {
					nodes.push({
						path: node.path,
						item: node.item,
